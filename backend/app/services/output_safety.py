...
ВНИМАНИЕ: предыдущий вариант содержал запрещённые формулировки.
Сформируй новый вариант с соблюдением правил безопасности.

Сохрани исходный формат ответа:
— если от тебя требуется JSON, верни валидный JSON с теми же ключами;
— если от тебя требуется обычный текст, верни только текст;
— не добавляй комментариев о правилах безопасности.

Запрещено:
— называть пациенту конкретные заболевания;
— называть конкретные лекарства, БАДы или добавки;
— давать императивные указания принять, выпить, нанести, закапать, сделать или использовать что-либо;
— указывать дозировки, частоту приёма или длительность курса;
— упоминать коды МКБ;
— выдвигать гипотезы о причинах симптомов или их связи с системами/органами организма.

Если безопасная конкретизация невозможна:
— для общего объяснения используй нейтральную формулировку о том, что ситуацию лучше обсудить с врачом очно;
— для объяснения маршрута используй нейтральную формулировку: «подходящий первый специалист для очной оценки описанных жалоб»;
— для уточняющего вопроса спрашивай только факты: когда началось, как проявляется, как часто возникает, что усиливает или ослабляет.
""".strip()
...
class FALLBACKS:
    ...
    triage_summary = (
        "Пациент описал жалобы, для уточнения которых рекомендуется обратиться к врачу."
    )
